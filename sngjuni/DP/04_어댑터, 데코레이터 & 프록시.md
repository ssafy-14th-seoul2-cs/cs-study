...
// Adapter
class TossPaymentAdapter implements Payment {
    private final TossPay tossPay = new TossPay();

    @Override
    public void pay(int amount) {
        tossPay.send(amount); // 변환
    }
}
...
2. **구조**
    ...
    ```java
    // Component (커피)
    //   └ ConcreteComponent (아메리카노)
    // Decorator (추가 옵션)
    //   └ ConcreteDecorator (우유 추가 / 시럽 추가 / 휘핑 추가 ...)
    ...
    abstract class CoffeeDecorator implements Coffee {
        protected Coffee coffee;  // 객체를 감싸서 확장
